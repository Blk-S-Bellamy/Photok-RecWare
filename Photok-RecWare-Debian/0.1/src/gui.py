...
class component_functions():
    ...
    def set_backup():
        ...
        pathway = QFileDialog.getExistingDirectory(caption="Select an uncompressed backup folder",
    ...
    def decrypt_backup():
        ...
        if current_backup['path'] != "":
            pathway = QFileDialog.getExistingDirectory(caption="Select a destination for the backup to be decrypted",
                                            options=QFileDialog.Option.DontUseNativeDialog,
                                            directory=HOME_PATH
                                            )
            if pathway == "":
...
class Window(QWidget):
    def __init__(self):
        ...
        self.resize(950, 650)
        self.setWindowTitle("Photok Recware 0.1")
        layout = QVBoxLayout()
