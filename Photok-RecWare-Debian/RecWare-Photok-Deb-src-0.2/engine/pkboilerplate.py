...
class util():
    # checks if the mime is added and adds it if not
    def addname(pathway):
        try:
            filename = os.path.basename(pathway)
            ext = os.path.splitext(filename)[1]
            print(ext)
            if ext == "":
                new_ext = util.type_resolver(pathway)
                newfile = f'{pathway}.{new_ext}'
                os.rename(Path(pathway), Path(newfile))
            else:
                pass
    ...
    def type_resolver(filepath):
        try:
            ...
            mime = mime.split('/')[1]  # get the ext without the seperator
            return mime
